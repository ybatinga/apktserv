package apkt.blockcypher.utils;
...
//    private static final Logger logger = Logger.getLogger(com.blockcypher.utils.sign.SignUtils.class);

    // created this method for when a multisig address receives funds more than once. 
    // It creates more than one output. For each output, two "tosign" are created.
    // And for each "tosign", two signatures are needed; therefore, if there are
    // two outputs, four signatures are needed. But the signatures need to be genererated
    // through this method
    public static void signWithHexKeyNoPubKeyMoreThanOneOutput(IntermediaryTransaction unsignedTransaction, String hexKey, String pubKey, int tosignIndex) {
        signMoreThanOneOutput(unsignedTransaction, hexKey, true, pubKey, tosignIndex);
    }
...
//                logger.info("Pushing Pub key for input");
    ...
    private static void signMoreThanOneOutput(
            IntermediaryTransaction unsignedTransaction, 
            String privateKey, 
            boolean isHex, 
            String pubKey,
            int tosignIndex) {
        ...
        String toSign = unsignedTransaction.getTosign().get(tosignIndex);

        unsignedTransaction.addPubKeys(pubKey);
        BigInteger[] components = signer.generateSignature(Hex.decode(toSign));
        BigInteger r = components[0];
        BigInteger s = components[1];
        // ensure Canonical
